...
public class GenreService {
	...
	public GenreDto getGenre(long id) {
		...
		return genreRepository.findById(id).map(this::mapGenreToGenreDto).orElseThrow(() -> new EntityNotFoundException(id, Genre.class));
	}

	@Transactional(readOnly = true)
	public GenreDto getGenre(String genre) {
		log.debug("get genre by genre name: {}", genre);
		return genreRepository.findByGenre(genre).map(this::mapGenreToGenreDto).orElseThrow(() -> new EntityNotFoundException(genre, Genre.class));
	}

	@Transactional(readOnly = true)
	public List<GenreDto> getAllGenres() {
		log.debug("get all genres");
		return genreRepository.findAll().stream().map(this::mapGenreToGenreDto).collect(Collectors.toList());
	}
